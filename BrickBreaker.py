...
- FONT = pg.font.Font("munro.ttf", 30) # change to "munro.ttf"
  BRICK_FONT = pg.font.Font("munro.ttf", 25)
TOOD:
...
FONT = pg.font.Font("fonts/munro/munro.ttf", 30)
BRICK_FONT = pg.font.Font("fonts/munro/munro.ttf", 25)
    ...
    def input(self):
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                ...
                show(f'FINGERDOWN{(event.x, event.y)}')
            elif event.type == pg.FINGERMOTION:
                show(f'FINGERMOTION{(event.dx, event.dy)}')
            elif event.type == pg.FINGERUP:
                show(f'FINGERUP{(event.x, event.y)}')
