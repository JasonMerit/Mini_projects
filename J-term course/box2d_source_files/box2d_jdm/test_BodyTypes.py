...
class BodyTypes (Framework):
    ...
    def newbullet(self, location_init, angle_radians_init, speed_control):
        # Speed
        v = 30
        #angle_degrees = angle_radians_init * 180/pi
        ...
        bullet_fixture = b2FixtureDef(shape=b2PolygonShape(box=(0.25, 0.25)), density=100.0)
    ...
    def Step(self, settings):
        ...
        if self.platform.type==b2_kinematicBody:
            ...
            if ((p.x < -10) or (p.x > 10)):    # (JDM)
                self.platform.linearVelocity *= -1

        # Every step (or second if use commented value), check the age of each bullet in the list. (JDM)
        self.time += 1.0/60.0
