...
class AirTrack:
    def __init__(self):
        ...
        self.env = Environment(W, 10)
        self.g = 9.8/20.0
    
    def update(self, dt):
        for car in self.cars:
            force = car.mass * self.g
            acc = force / car.mass
            v1 = car.speed + acc * dt
            v = (car.speed + v1) / 2.0
            dx = v * dt
    ...
    def make_cars(self, demo):
        if demo == 1:
            car1 = Detroit((50, 100), W // 2, 0.1, RED)
            car2 = Detroit((50, 100), 0, 0.2, BLUE)
        elif demo == 2:
            car1 = Detroit((50, 100), W, -0.2, RED)
            car2 = Detroit((50, 100), 0, 0.2, BLUE)
...
class Environment:
    ...
    def pixels_to_meters(self, pixels):
        return float(pixels) * self.pix2met
    
    def meters_to_pixels(self, meters):
        return int(round(meters * self.met2pix))
